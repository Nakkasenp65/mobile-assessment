...
const RefinanceService = ({ service, theme, isSelected }: RefinanceServiceProps) => {
  const { id, title, description, icon: Icon, price: refinancePrice } = service;
    ...
    >
      <AccordionTrigger className="w-full cursor-pointer p-4 text-left hover:no-underline">
        <div className="flex w-full items-center gap-4">
          <div
            className={cn(
              "flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-purple-500 to-purple-700 shadow-sm dark:bg-zinc-800",
            )}
          >
          ...
          <div className="ml-2 text-right">
            <p className="text-foreground text-lg font-bold">{THB(refinancePrice)}</p>
          </div>
        </div>
      </AccordionTrigger>
      {/* Benefits Section */}
      ...
      <AccordionContent className="px-4 pb-4">
        <div className="mt-2 space-y-6 border-t pt-4 dark:border-zinc-700/50">
          ...
          {/* โครงสร้างของฟอร์มทั้งหมดถูกนำมาวางไว้ที่นี่ */}
          <motion.div
            initial="initial"
            animate="animate"
            variants={{
          ...
          >
            <motion.div variants={formVariants} className="space-y-4">
              ...
              <div className="space-y-2">
                {/* เพื่อป้องกันการชนกันของ id และ label เมื่อมี Accordion หลายอันในหน้าเดียวกัน */}
              ...
              {documentUploadDetails && (
                ...
                >
                  <Label htmlFor={`document-upload-${id}`} className="font-semibold">
