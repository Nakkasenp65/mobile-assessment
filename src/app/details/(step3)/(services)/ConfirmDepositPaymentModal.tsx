...
export default function ConfirmDepositPaymentModal({
  isOpen,
  setIsOpen,
  title,
  onConfirm,
  ...
  return (
    <motion.div
      ...
      className={clsx(
        "",
        isOpen
          ? "fixed inset-0 z-50 flex h-screen items-center justify-center backdrop-blur-xs"
          : "hidden",
      )}
    >
          ...
          <div className="flex flex-col gap-4 px-6 py-4 md:flex-row md:items-center md:gap-6">
            ...
            <div className="flex min-w-0 flex-2 flex-col">
              <DialogHeader className="w-full p-0">
                <DialogTitle className="mb-2 text-xl font-semibold text-gray-900 md:text-xl">
                  {title}
                </DialogTitle>
              </DialogHeader>
              ...
              <DialogDescription className="text-bold mt-2 text-sm md:text-base">
                ...
                เพื่อยืนยันการนัดหมาย โปรดตรวจสอบยอดและดำเนินการต่อ
              </DialogDescription>

              <div className="mt-4 flex items-center gap-3">
                <div
                  className={`text-primary border-primary flex gap-6 rounded-lg border bg-white px-3 py-2 text-sm font-medium`}
                >
                  <div className="flex flex-col items-center">
                    <span className="text-xs text-nowrap">ยอดมัดจำ</span>
                    <span className="text-lg font-semibold md:text-2xl">200฿</span>
                  </div>
                  <div>
                    เมื่อชำระแล้วระบบจะนำท่านไปยังหน้าชำระเงิน ยอดมัดจำจะได้รับคืนหลังจบการบริการ
                  </div>
                </div>
              </div>
            </div>
